...
function updateClock() {
  ...
  document.getElementById('live-clock').textContent =
    new Intl.DateTimeFormat('en-US', { ...tz, hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: true }).format(now) + ' ET';
  document.getElementById('live-date').textContent =
    new Intl.DateTimeFormat('en-US', { ...tz, weekday: 'short', month: 'short', day: 'numeric' }).format(now);
}
updateClock();
setInterval(updateClock, 1000);

// ── Typewriter (cycles phrases in the brand name)
...
// ── Contact form (Google Apps Script, no-cors)
...
contactForm.addEventListener('submit', async e => {
  ...
  try {
    await fetch(
      'https://script.google.com/macros/s/AKfycbzTi-Pcn7tzisi2yP7XvxQ6JEQmPoI-9X3ebc4RF5NBCIdC8P0SQMUTlaQ1xMAgcAGv/exec',
...
// ── Google Calendar iframe — adblock fallback
...
// ── Weather (Open-Meteo API — no key, no tracking, adblock-safe)
const WEATHER_LAT = 35.1085;
const WEATHER_LON = -77.0441;
...
async function fetchWeather() {
  ...
  try {
    const url = `https://api.open-meteo.com/v1/forecast?latitude=${WEATHER_LAT}&longitude=${WEATHER_LON}&current=temperature_2m,apparent_temperature,relative_humidity_2m,wind_speed_10m,weather_code&temperature_unit=fahrenheit&wind_speed_unit=mph&timezone=America%2FNew_York`;
